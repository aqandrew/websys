...
// Part 1 - Add subclasses for Subtraction, Multiplication and Division here
    ...
    public function operate() {
			$factorial = $this->operand_1;
			for ($factor = $this->operand_1 - 1; $factor > 0; $factor--) {
				$factorial *= $factor;
			}
      return $factorial;
    }
...
// Put the code for Part 2 here  \/
      ...
      $op = new Division($o1, $o2);
		...
		elseif (isset($_POST['cube']) && $_POST['cube'] == 'Cube') {
      $op = new Cube($o1);
    }
		elseif (isset($_POST['fact']) && $_POST['fact'] == 'Factorial') {
      $op = new Factorial($o1);
    }
// End of Part 2   /\
  }
...
<body>
	<h1>PHC Hypertext Calculator</h1>
  <pre id="result">
  <?php 
    if (isset($op)) {
      try {
